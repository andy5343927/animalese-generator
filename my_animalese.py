...
def main():
    ...
    for index, ltr in enumerate(keys):
        ...
        if num < 10:
            num = "0" + str(num)
        sounds[ltr] = "./sounds/" + argument.pitch + \
            "/sound" + str(num) + ".wav"
    ...
    for index, sound in enumerate(infiles):
        ...
        new_sound = tempsound._spawn(tempsound.raw_data, overrides={
            "frame_rate": new_sample_rate})
        new_sound = new_sound.set_frame_rate(
            argument.uniform_sample_rate)  # set uniform sample rate
